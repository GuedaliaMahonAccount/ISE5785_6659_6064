// renderer/StreetSceneThreadsTest.java
package renderer;
 ...
 * All street lamps face toward the road center.
 * The scene uses simplified facades (no windows), reduced lighting, and soft-area shadows.
 * The camera is placed closer to capture more detail, with multi-threaded rendering enabled.
 * BVH acceleration is disabled—raw geometries are used.
 */
public class StreetSceneThreadsTest {
    ...
    public void streetScene() {
        // 1) Build a flat list of all primitives
        ...
        // Left sidewalk (slightly lighter for contrast)
        geometries.add(new Polygon(
                new Point(-45, 1.2, 200),
                new Point(-25, 1.2, 200),
                new Point(-25, 1.2, -500),
        ...
        // Attach all primitives directly (no BVH)
        ...
        // Soft-area street lamps
        for (int i = 0; i < 6; i++) {
            double zL = -40 - i * 50, zR = zL + 25;
            scene.lights.add(
                    new PointLight(new Color(100,90,80), new Point(-50, 8 + 1.2, zL))
                            .setKl(0.001).setKq(0.0005).setRadius(10.0).setNumSamples(5)
            );
            scene.lights.add(
                    new PointLight(new Color(100,90,80), new Point( 55, 8 + 1.2, zR))
                            .setKl(0.001).setKq(0.0005).setRadius(10.0).setNumSamples(5)
            );
        }
        ...
        // ===== Camera Setup (multi-threaded) & Anti-aliasing =====
        ...
        camera.writeToImage("streetThreads");
        System.out.println("Image written to file: streetThreads.png");
    }
